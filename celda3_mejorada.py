...
for ejemplar in ejemplares:
    ...
    if APLICAR_SEPARACION and plano_info_verificacion is not None:
        ...
        # UMBRAL DE RE-ORIENTACIÓN: 10° (según recomendación del usuario)
        if angulo_validacion < 5:
            print(f"    ✓ Excelente alineación (< 5°)")
        elif angulo_validacion < 10:
            print(f"    ✓ Buena alineación (< 10°)")
        else:
            print(f"    ⚠️ ADVERTENCIA: Pobre alineación ({angulo_validacion:.2f}°)")
            print(f"    - La bbox no está bien orientada respecto a la antena")
            print(f"    - Umbral de re-orientación: 10° (se recalcula si > 10°)")
            ...
            if angulo_validacion_nueva < 5:
                print(f"      ✓ Excelente alineación alcanzada (< 5°)")
            elif angulo_validacion_nueva < 10:
                print(f"      ✓ Buena alineación alcanzada (< 10°)")
            else:
                print(f"      ⚠️ Alineación mejorada pero aún requiere ajuste (> 10°)")
        ...
        if datos.get('bbox_reorientada', False):
            ...
            if angulo_validacion is not None:
                if angulo_validacion < 5:
                    ...
                    status = "⚠️ Requiere re-orientación"
